<?php
  // Language: Spanish/Espanol/Castellano

  // Main strings
  $l_login = "Ingresar";
  $l_logout = "Salir";
  $l_pleaselogin = "Ingrese al sistema";
  $l_username = "Nombre de Usuario";
  $l_password = "Contrase&ntilde;a";

  // General Strings

  // Menu Option
  $l_menu = "Men&uacute;";
  $l_dashboard = "Tablero";
  $l_whatis = "&iquest;Qu&eacute; es?";
  $l_updatedata = "Actualizar mis datos";
  $l_updatepass = "Cambiar mi contrase&ntilde;a";
  $l_admusers = "Adm. Usuarios";
  $l_admgroups = "Adm. Grupos";
  $l_viewlogs  ="Ver Logs";
  $l_admsysusers = "Adm. Usuarios Sistema";

  // Dashboard
  $l_users = "Usuarios";
  $l_groups = "Grupos";
  $l_loggedin = "Logueados";
  $l_userprocesses = "Proc.Usuario";
  $l_server = "Servidor";
  $l_datetime = "Fecha y Hora";
  $l_connectedfrom = "Conectado desde";
  $l_user = "Usuario";
  $l_changelog = "Bit&aacute;cora de Cambios";

  // No Access
  $l_noaccess = "No tiene acceso!"; 
  $l_noaccess_notice = "Este intento de acceso ser&aacute; registrado.";

  // Common
  $l_goback = "Volver";
  $l_username = "Nombre de Usuario";
  $l_name = "Nombre";
  $l_idc = "C&eacute;dula de Identidad";
  $l_dob = "Fecha de Nacimiento";
  $l_email = "E-Mail";
  $l_phonenumber = "Tel&eacute;fono";
  $l_city = "Ciudad";
  $l_address = "Direcci&oacute;n";
  $l_department = "Departamento";
  $l_update = "Actualizar";
  $l_reset = "Reiniciar";
  $l_yesconfirm = "Si, confirmo!";
  $l_no = "No";
  $l_yes = "Si";
  $l_remove = "Remove";
  $l_add = "Adicionar";
  $l_today = "Hoy";
  $l_search = "Buscar";

  // Errors & Evaluations (Verifications)
  $l_verifyingvars = "Verificando variables";
  $l_errorsfound = "Encontr&eacute; los siguientes errores";
  $l_nullmail = "correo";
  $l_nullphonenumber = "tel&eacute;fono";
  $l_nullday = "d&iacute;a de nacimiento";
  $l_nullmonth = "mes de nacimiento";
  $l_nullyear = "a&ntilde;o de nacimiento";
  $l_nullidc = "de la c&eacute;dula de identidad";
  $l_nulladdress = "de la direcci&oacute;n";
  $l_nullpassword = "de la contrase&ntilde;a";
  $l_nullnewpassword = "de la contrase&ntilde;a nueva";
  $l_nullnewpassword2 = "de la confirmaci&oacute;n de la contrase&ntilde;a nueva";
  $l_nullquotasb = "de la quota soft de bloques";
  $l_nullquotahb = "de la quota hard de bloques";
  $l_nullquotasi = "de la quota soft de I-Nodos";
  $l_nullquotahi = "de la quota hard de I-Nodos";
  $l_nullinfo = "de la informaci&oacute;n";
  $l_nullhome = "del directorio del usuario";
  $l_nullshell = "del shell del usuario";
  $l_nullquota = "de la quota principal del usuario";
  
  $l_unmatchedpasswords = "de las contrase&ntilde;as";
  $l_repdbmail = "del correo";
  $l_errorbefore = "El campo";
  $l_errorafter = "no puede ser nulo.";
  $l_error2before = "Los campos";
  $l_error2after = "no son iguales.";
  $l_erroremailbefore = "La direcci&oacute;n";
  $l_erroremailafter = "no es un correo v&aacute;lido.";
  $l_errordatebefore = "La fecha";
  $l_errordateafter = "no es v&aacute;lida.";
  $l_errornumericbefore = "El campo del";
  $l_errornumericafter = "debe ser num&eacute;rico.";
  $l_errorrangebefore = "El camnpo del";
  $l_errorrangeafter = "no est&aacute; en el rango permitido.";
  $l_errorusernameexists = "El nombre de usuario ya existe en el sistema.";
  $l_errorgrouonamexists = "El nombre del grupo ya existe en el sistema.";

  // UpdateData
  $l_ud_pleaseupdate = "Por favor, actualice su informaci&oacute;n:";  
  $l_ud_pleaseconfirm = "Por favor, confirme los siguientes cambios:";
  $l_ud_updatingdata = "Actualizando sus datos ...";
  $l_ud_updatedata = "Actualizaci&oacute;n de datos";

  // UpdatePass
  $l_up_pleaseupdate = "Por favor, actualice su contrase&ntilde;a:";
  $l_up_currentpass = "Contrase&ntilde;a actual";
  $l_up_newpass = "Nueva contrase&ntilde;a";
  $l_up_newpass2 = "Repita Nueva contrase&ntilde;a";
  $l_up_updatingpass = "Actualizando su contrase&ntilde;a ...";
  $l_up_qualitypass = "Calidad de la contrase&ntilde;a";
  $l_up_updatepass = "Actualizaci&oacute;n de contrase&ntilde;a";
  $l_up_nomatchpass = "La contrase&ntilde;a enviada no coincide con su contrase&ntilde;a actual.";
  $l_up_onlychangepass = "<strong>Atenci&oacute;n:</strong> Su contrase&ntilde;a ha expirado, debe configurar una nueva contrase&ntilde;a.";

  // WhatIs
  $l_wi_text  = "$surp_sitename v.$surp_sitever es la interfaz web creada para configurar las opciones disponibles en el servidor.<br />\n";
  $l_wi_text .= "Originalmente fue escrito por $surp_autname<br />\n";

  // Admin Users
  $l_au_userremoval = "Eliminaci&oacuten de usuario ...";
  $l_au_uid = "UID";
  $l_au_gid = "GID";
  $l_au_username = "Username";
  $l_au_info = "Info";
  $l_au_home = "Home";
  $l_au_shell = "Shell";
  $l_au_groups = "Grupos";
  $l_au_quota  ="Quota";
  $l_au_important = "Importante";
  $l_au_taskscheduled = "La tarea ha sido programada";
  $l_au_taskid = "Tarea ID";
  $l_au_userremovalconf = "Por favor confirme que desea eliminar el siguiente usuario:";
  $l_au_userquota = "Modificaci&oacute;n de la quota del usuario";
  $l_au_quotasb = "Quota Soft Bloques";
  $l_au_quotahb = "Quota Hard Bloques";
  $l_au_quotasi = "Quota Soft I-Nodos";
  $l_au_quotahi = "Quota Hard I-Nodos";
  $l_au_usermodconf = "Por favor confirme la siguiente modificaci&oacute;n de usuario:";
  $l_au_quotabnotice = "Quotas en bytes(bloques)";
  $l_au_groupsmod = "Modificaci&oacute;n de la pertenencia de grupos del usuario:";
  $l_au_groupscurrent = "Grupos actuales";
  $l_au_groupsnew = "Nuevos grupos";
  $l_au_shellmod = "Modificaci&oacute;n de shell del usuario:";
  $l_au_infomod = "Modificaci&oacute;n de la info del usuario:";
  $l_au_infocurrent = "Info actual";
  $l_au_infonew = "Nueva info";
  $l_au_useradd = "Adicionando el usuario al sistema...";
  $l_au_useraddconf = "Por favor confirme la siguiente adici&oacute;n:";
  $l_au_useraddition = "Creaci&oacute;n de usuario:";
  $l_au_usernamenotice = "(No debe existir en el sistema)";
  $l_au_passwdnotice = "(Ser&aacute; mostrada, para que el usuario pueda configurar)";
  $l_au_infonotice = "(Formato: Nombre completo,Oficina,tel&eacute;fono)";
  $l_au_homenotice = "(Si deja en blanco se configurar&aacute; en /home/USUARIO)";
  $l_au_viewuser = "Informaci&oacute;n de usuario";
  $l_au_userscurrent = "Usuarios Actuales del Sistema:";
  $l_au_name = "Nombre";
  $l_au_operations = "Operaciones";
  $l_au_removeuser = "Presione ac&aacute; para eliminar el usuario";

  // Admin Groups
  $l_ag_groupname = "Nombre del Grupo";
  $l_ag_groupmembers ="Miembros del grupo";
  $l_ag_operations = "Operaciones";
  $l_ag_viewgroup = "Informaci&oacute;n de grupo";
  $l_ag_removegroup = "Presione ac&aacute; para eliminar el grupo";
  $l_ag_groupremovalconf = "Por favor confirme que desea eliminar el siguiente grupo:";
  $l_ag_groupadd = "Adici&oacute;n de nuevo grupo:";
  $l_ag_groupaddconf = "Por favor confirme la siguiente adici&oacute;n de grupo:";

  // ViewLogs
  $l_vl_opdetail = "Detalle de Operaci&oacute;n";
  $l_vl_datetime = "Fecha Hora";
  $l_vl_command = "Comando";
  $l_vl_output = "Salida";
  $l_vl_returnvar = "Retorno;";
  $l_vl_opinfo = "Informaci&oacute;n de Operaci&oacute;n:";
  $l_vl_operation = "Operaci&oacute;n";
  $l_vl_r_user = "Iniciada por";
  $l_vl_d_user = "Cuenta destino";
  $l_vl_optype = "Tipo";
  $l_vl_flags = "Opciones";
  $l_vl_result = "Resultado";
  $l_vl_status = "Estado";
  $l_vl_viewdetail = "Ver detalle de la operaci&oacute;n";
  $l_vl_searchresult = "Resultados de su b&uacute;squeda";
  $l_vl_daterange = "Rango de Fechas";
  $l_vl_anyuser = "Cualquier usuario";
  $l_vl_anygroup = "Cualquier grupo";
  $l_vl_opover = "Operaciones hechas sobre";
  $l_vl_optypef = "Tipo de Operaci&oacute;n";
  $l_vl_shresults = "Resultados";
  $l_vl_id = "ID";
  $l_vl_opby ="Por";
  $l_vl_opovers = "Sobre";
  $l_vl_opid = "Op";
  $l_vl_detailsearch = "Por favor detalle su b&uacute;squeda:";
  $l_vl_opfrom ="Operaciones hechas por";
  $l_vl_anyop = "Cualquier Op";


  // Aditional stuff
  $phpFileUploadErrors = array(
    0 => 'No hubo error, el fichero subio exitosamente',
    1 => 'El fichero subido excede la directiva upload_max_filesize en el php.ini',
    2 => 'El fichero subido excede la directiva MAX_FILE_SIZE especificada en el formulario HTML',
    3 => 'El fichero solo fue subido parcialmente',
    4 => 'No se subio un fichero',
    6 => 'Falta un folder temporal',
    7 => 'No pude escribir el fichero en el disco.',
    8 => 'Una extension PHP detuvo la subida del fichero.',
  );

  // Months
  $l_m01 = "Enero";
  $l_m02 = "Febrero";
  $l_m03 = "Marzo";
  $l_m04 = "Abril";
  $l_m05 = "Mayo";
  $l_m06 = "Junio";
  $l_m07 = "Julio";
  $l_m08 = "Agosto";
  $l_m09 = "Septiembre";
  $l_m10 = "Octubre";
  $l_m11 = "Noviembre";
  $l_m12 = "Diciembre";

  // Short months
  $l_sm01 = "Ene";
  $l_sm02 = "Feb";
  $l_sm03 = "Mar";
  $l_sm04 = "Abr";
  $l_sm05 = "May";
  $l_sm06 = "Jun";
  $l_sm07 = "Jul";
  $l_sm08 = "Ago";
  $l_sm09 = "Sep";
  $l_sm10 = "Oct";
  $l_sm11 = "Nov";
  $l_sm12 = "Dic";

  // Separators
  $l_procdate_sep = "/";
  $l_procdate2_sep = "-";
  $l_procdate3_sep = "/";

  // Days of week
  $l_dow0 = "Domingo";
  $l_dow1 = "Lunes";
  $l_dow2 = "Martes";
  $l_dow3 = "Mi&eacute;rcoles";
  $l_dow4 = "Jueves";
  $l_dow5 = "Viernes";
  $l_dow6 = "S&aacute;bado";

  $l_sdow0 = "Do";
  $l_sdow1 = "Lu";
  $l_sdow2 = "Ma";
  $l_sdow3 = "Mi";
  $l_sdow4 = "Ju";
  $l_sdow5 = "Vi";
  $l_sdow6 = "Sa";
?>
